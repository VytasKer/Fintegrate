...
class CustomerStatusChange(BaseModel):
    """Schema for PATCH /customer/change-status and admin status endpoints"""

    customer_id: UUID
    status: str = Field(
        ...,
        pattern="^(ACTIVE|INACTIVE|PENDING_AML|BLOCKED)$",
        description="Customer status: ACTIVE, INACTIVE, PENDING_AML, or BLOCKED",
    )
...
class EventResendRequest(BaseModel):
    ...
    event_types: Optional[List[str]] = Field(
        None, description="Filter by specific event types (e.g., ['customer_creation'])"
    )
...
class EventConfirmDeliveryRequest(BaseModel):
    ...
    status: str = Field(
        ..., pattern="^(received|processed|failed)$", description="Processing status: received, processed, or failed"
    )
...
class EventRedeliverRequest(BaseModel):
    ...
    max_try_count: Optional[int] = Field(
        None, ge=1, le=10, description="Skip events that exceeded this delivery retry count"
    )
...
class ConsumerCreate(BaseModel):
    ...
    name: str = Field(
        ...
        max_length=230,
        pattern="^[a-z0-9_]+$",
        description="Consumer name (alphanumeric + underscore)",
    )
...
class AnalyticsSnapshot(BaseModel):
    ...
    analytics_id: UUID
    consumer_id: Optional[UUID] = Field(
        None, description="Consumer UUID for per-consumer snapshots, null for global snapshots"
    )
