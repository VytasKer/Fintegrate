    ...
    try:
        ...
        try:
            ...
            response = requests.post(
                f"{CUSTOMER_SERVICE_URL}/events/confirm-delivery", json=confirmation_payload, timeout=5
            )
        ...
        if event_id:
            try:
                ...
                requests.post(f"{CUSTOMER_SERVICE_URL}/events/confirm-delivery", json=confirmation_payload, timeout=5)
        ...
        if retry_count >= 3:
            ...
            new_properties = pika.BasicProperties(headers=new_headers, delivery_mode=2, content_type="application/json")
            ...
            ch.basic_publish(
                exchange="customer_events", routing_key=method.routing_key, body=body, properties=new_properties
            )
...
def main():
    ...
    try:
        ...
        channel.queue_bind(exchange="customer_events", queue=queue_name, routing_key=f"customer.*.{consumer_name}")

        # Bind DLQ to exchange
        channel.queue_bind(exchange="customer_events", queue=dlq_name, routing_key=f"customer.dlq.{consumer_name}")
