...
def process_historical_period(**context):
    ...
    start_date = datetime.fromisoformat(
        context["task_instance"].xcom_pull(task_ids="validate_params", key="start_date")
    )
    ...
    try:
        ...
        while current_date < end_date:
            ...
            for consumer_id in all_consumer_ids:
                cust_metrics = customer_metrics.get(
...
with DAG(
    ...
    params={
        "start_date": Param(
            "2025-01-01", type="string", description="Start date for backfill (YYYY-MM-DD format)", title="Start Date"
        ),
        "end_date": Param(
            "2025-01-31", type="string", description="End date for backfill (YYYY-MM-DD format)", title="End Date"
        ),
        "interval": Param(
            "daily",
            type="string",
